...
filename = 'HBM_SLinverter_test.csv';
test_prefix = 'SLinvtest0'
% ------------------------------------------------------------
...
high_vds_on_t = [1.895 1.896];

% High-side VDS during turn-off
high_vds_off_t = [1.2 1.201];

% Switch-node voltage
switch_t = [1.1 1.15];

% Gate current plot
gate_curr_t = [1.099 1.102];    % [ms] <-- choose independently
% ------------------------------------------------------------
...
yyaxis left
plot(t_plot, Vload_plot, 'LineWidth', 1.2);
...
xlabel('Time [ms]');
title('Load Voltage and Current');
xlim(load_t);
grid on;

legend('Load Voltage', 'Load Current', 'Location', 'best');
